...
def autenticador(username, password):
    '''Función encargada en mi JWT de validar las credenciales, valida si son ingresadas correctamente y luego valida si es el usuario'''
...
def identificador(payload):
    '''Sirve para que una vez el usuario, envie la token y quiera realizar una petición a una ruta protegida, esta función será encargada de identificar a dicho usuario y devolver su información'''
    print(payload)
